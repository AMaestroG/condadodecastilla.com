    ...
    def find_uncrawled_links(self, limit: int = 10) -> list[str]:
        ...
        for link_data in all_links:
            ...
            if not resource:
                is_uncrawled = True
                print(
                    f"  Target URL {target_url} not found in DB (considered uncrawled)."
                )
            else:
                ...
                if (
                    not content
                    or content.strip() == ""
                    or content == "N/A (placeholder)"
                ):
                    is_uncrawled = True
                    print(
                        f"  Target URL {target_url} found but content is missing/placeholder (considered uncrawled)."
                    )
                else:
                    print(
                        f"  Target URL {target_url} found and has content (considered crawled)."
                    )
...
if __name__ == "__main__":
    ...
    db.add_or_update_resource(
        {
            ...
            "metadata": {"title": "Crawled Page Title"},
    ...
    db.add_or_update_resource(
        {
            "id": no_content_res_id,
            "url": no_content_res_url,
            "content": "",  # Empty content
            "metadata": {"title": "No Content Page Title"},
            "last_crawled_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_or_update_resource(
        {
            "id": placeholder_content_res_id,
            "url": placeholder_content_res_url,
            "content": "N/A (placeholder)",
            "metadata": {"title": "Placeholder Content Page Title"},
            "last_crawled_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": crawled_res_url,
            "target_url": no_content_res_url,
            "anchor_text": "Link to no-content",
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": crawled_res_url,
            "target_url": placeholder_content_res_url,
            "anchor_text": "Link to placeholder-content",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": crawled_res_url,
            "target_url": non_existent_url1,
            "anchor_text": "Link to non-existent 1",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": crawled_res_url,
            "target_url": non_existent_url2,
            "anchor_text": "Link to non-existent 2",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": no_content_res_url,
            "target_url": crawled_res_url,
            "anchor_text": "Link back to crawled",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    assert db.resource_exists(
        non_existent_url1
    )  # Should have been created as placeholder by add_link
    assert db.resource_exists(
        non_existent_url2
    )  # Should have been created as placeholder by add_link
