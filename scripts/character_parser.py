...
def parse_character_html(file_path):
    ...
    try:
        ...
        if category_name_candidate and category_name_candidate.lower() != "personajes":
            ...
            category_name_candidate.lower() == "personajes"
        ):  # File is directly under "personajes"
    ...
    if not content_wrapper:
        content_wrapper = soup.find(
            "div", class_="page-content-block personaje-detail-content"
        )
    ...
    if content_wrapper:
        ...
        for p in content_wrapper.find_all("p", recursive=True):
            ...
            if (
                not p.find_parent("table")
                and not p.find_parent("blockquote")
                and not p.find_parent(class_="alert")
            ):
                ...
                if len(text) > 50:  # Assume an intro paragraph has some substance
                    ...
                    break
        if intro_paragraph:
            character_data["bio_snippet"] = (
                intro_paragraph[:300] + "..."
                if len(intro_paragraph) > 300
                else intro_paragraph
            )
        elif content_wrapper.find(
            "p"
        ):  # Fallback to the very first paragraph if no better candidate
            first_p_text = content_wrapper.find("p").get_text(strip=True)
            character_data["bio_snippet"] = (
                first_p_text[:300] + "..." if len(first_p_text) > 300 else first_p_text
            )

    # 3. Extract Key Facts (Hitos Importantes)
    hitos_heading = soup.find(
        ["h2", "h3"], string=lambda text: text and "Hitos Importantes" in text
    )
    if not hitos_heading:  # Try another common heading
        hitos_heading = soup.find(
            ["h2", "h3"], string=lambda text: text and "Legado y Relevancia" in text
        )
            ...
            while next_element and next_element.name == "p":
                ...
                if fact_text:
                    ...
                    if ". " in fact_text or "; " in fact_text:
                        facts = [
                            f.strip()
                            for f in fact_text.replace("; ", ". ").split(". ")
                            if f.strip()
                        ]
...
if __name__ == "__main__":
    ...
    dummy_html_path = os.path.join(
        "personajes", dummy_category_name, "dummy_character.html"
    )
    ...
    if parsed_data_no_hitos:
        ...
        print(
            f"Category: {parsed_data_no_hitos['category']} (Expected: personajes or Unknown based on path)"
        )
    else:
