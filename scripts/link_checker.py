        ...
        if tag == "a":
            ...
            if "href" in attrs:
                ...
                if not (
                    ...
                    or href.startswith("#")  # Ignore pure fragment links
                ):
...
def resolve_link(source_html_path, link_href, repo_root):
    ...
    if link_href_no_fragment.startswith("/"):
        ...
        resolved_path_no_fragment = os.path.normpath(
            os.path.join(repo_root, link_href_no_fragment.lstrip("/"))
        )
    else:
        # Relative link
        source_dir = os.path.dirname(source_html_path)
        resolved_path_no_fragment = os.path.normpath(
            os.path.join(source_dir, link_href_no_fragment)
        )
...
def main():
    ...
    for html_file_path in html_files:
        ...
        for link_href in parser.links:
            resolved_path_no_fragment, resolved_path_for_reporting = resolve_link(
                html_file_path, link_href, repo_root
            )
            ...
            if not os.path.exists(resolved_path_no_fragment):
                ...
                report_resolved_path = os.path.relpath(
                    resolved_path_for_reporting, repo_root
                )
