...
class GraphDBInterface:
    ...
    def _load_from_file(self):
        ...
        try:
            ...
            print(
                f"Database file {self.db_filepath} not found. Starting with an empty database."
            )
        except json.JSONDecodeError as e:
            print(
                f"Error decoding JSON from {self.db_filepath}: {e}. Starting with an empty database."
            )
        except IOError as e:
            print(
                f"Error loading database state from {self.db_filepath}: {e}. Starting with an empty database."
            )
    ...
    def add_link(self, link_data: dict) -> None:
        ...
        for edge in self._edges:
            if (
                edge.get("source_url") == source_url
                and edge.get("target_url") == target_url
            ):
                print(
                    f"Link from {source_url} to {target_url} already exists. Skipping."
                )
                return
...
if __name__ == "__main__":
    ...
    if retrieved_res_updated:
        print(
            f"Updated content for page1: {retrieved_res_updated.get('content')}, Status: {retrieved_res_updated.get('metadata', {}).get('status')}"
        )
    ...
    link2_data = {
        "source_url": "http://example.com/page1",
        "target_url": "http://example.com/page3",
        "anchor_text": "Link to Page 3 (non-existent yet)",
    }
    ...
    print(
        f"Links from page1: {[(l['source_url'], l['target_url']) for l in links_from_page1]}"
    )

    # Get links to a resource
    links_to_page2 = db.get_links_to_resource("http://example.com/page2")
    print(
        f"Links to page2: {[(l['source_url'], l['target_url']) for l in links_to_page2]}"
    )

    links_to_page3 = db.get_links_to_resource(
        "http://example.com/page3"
    )  # Page 3 was a placeholder
    print(
        f"Links to page3: {[(l['source_url'], l['target_url']) for l in links_to_page3]}"
    )
    ...
    link3_data = {
        ...
        "target_url": "http://example.com/page5",
        "anchor_text": "Link from new to new",
    }
    ...
    for r_url, r_data in db._nodes.items():
        print(
            f"  URL: {r_url}, ID: {r_data['id']}, Content: {r_data.get('content', 'N/A')[:30]}..., Crawled: {r_data.get('last_crawled_at')}"
        )
    print("All Links:")
    for l_info in db._edges:
        print(
            f"  ID: {l_info['id']}, From: {l_info['source_url']} (ID: {l_info['source_resource_id']}) -> To: {l_info['target_url']} (ID: {l_info.get('target_resource_id', 'N/A')}), Anchor: {l_info.get('anchor_text', 'N/A')}"
        )
    ...
    db_loaded = (
        GraphDBInterface()
    )  # This will attempt to load from knowledge_graph_db.json
    print(
        f"Loaded {len(db_loaded.get_all_resources())} resources and {len(db_loaded.get_all_links())} links on second init."
    )
