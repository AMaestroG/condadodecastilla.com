...
def handle_add_url(url_to_crawl: str):
    ...
    page_title, extracted_links_info = crawler.parse_html(
        raw_html_content, url_to_crawl
    )
    print(
        f"Parsed HTML. Title: '{page_title}'. Found {len(extracted_links_info)} links."
    )
    ...
    print(
        f"Resource for {url_to_crawl} added/updated in the database with ID: {resource_id}."
    )
    ...
    if extracted_links_info:
        ...
        print(
            f"Added {len(extracted_links_info)} links originating from {url_to_crawl}."
        )
    else:
...
def main():
    ...
    add_parser = subparsers.add_parser(
        "add_url", help="Crawl a URL and add it to the graph."
    )
    add_parser.add_argument(
        "url", type=str, help="The URL to crawl (e.g., http://example.com)."
    )
    add_parser.set_defaults(func=lambda args: handle_add_url(args.url))

    # Command: show_resource
    show_parser = subparsers.add_parser(
        "show_resource", help="Show a specific resource by URL."
    )
    ...
    list_parser = subparsers.add_parser(
        "list_resources", help="List all resources in the graph."
    )
    list_parser.set_defaults(func=lambda args: handle_list_resources())

    # Command: find_uncrawled
    uncrawled_parser = subparsers.add_parser(
        "find_uncrawled", help="Find links pointing to uncrawled resources."
    )
    uncrawled_parser.add_argument(
        "--limit",
        type=int,
        default=10,
        help="Maximum number of uncrawled URLs to return.",
    )
    ...
    consistency_parser = subparsers.add_parser(
        "run_consistency", help="Run consistency checks on the graph."
    )
