...
# Assuming graph_db_interface.py is in the same directory
...
class ConsistencyAnalyzer:
    ...
    def check_resource_completeness(self, resource_data: dict) -> dict:
        ...
        if not processed_content or len(processed_content.strip()) <= 50:
            issues.append(
                f"Processed content is too short (length: {len(processed_content.strip())})."
            )
            complete = False
    ...
    def check_link_topical_coherence(self, link_data: dict, threshold=0.1) -> dict:
        ...
        if (
            not source_content
            or source_content.strip() == ""
            or source_content == "N/A (placeholder)"
        ):
            result["reason"] = (
                f"Missing or placeholder content for source resource {source_url}."
            )
            return result
        if (
            not target_content
            or target_content.strip() == ""
            or target_content == "N/A (placeholder)"
        ):
            result["reason"] = (
                f"Missing or placeholder content for target resource {target_url}."
            )
            return result
        ...
        result["score"] = 1.0
        result["reason"] = (
            "Placeholder: Both source and target pages have content. Actual NLP similarity check needed."
        )
...
if __name__ == "__main__":
    ...
    res_placeholder = {
        "id": res_placeholder_id,
        "url": "http://example.com/placeholder-page",
        "content": "N/A (placeholder)",
        "metadata": {"title": "Placeholder Page", "status": "placeholder"},
        "last_crawled_at": datetime.utcnow().isoformat(),
    }
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": "http://example.com/valid",
            "target_url": "http://example.com/no-title",
            "anchor_text": "Link to no-title page",
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": "http://example.com/valid",
            "target_url": "http://example.com/short-content",
            "anchor_text": "Link to short-content page",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": "http://example.com/valid",
            "target_url": "http://example.com/placeholder-page",
            "anchor_text": "Link to placeholder page",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": "http://example.com/placeholder-page",
            "target_url": "http://example.com/valid",
            "anchor_text": "Link from placeholder page",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    db.add_link(
        {
            "id": str(uuid.uuid4()),
            "source_url": "http://example.com/valid",
            "target_url": "http://example.com/non-existent-target",
            "anchor_text": "Link to non-existent page",
            "created_at": datetime.utcnow().isoformat(),
        }
    )
    ...
    if not issues:
        ...
        for issue in issues:
            ...
            if "link_id" in issue:
                print(
                    f"    Link ID: {issue['link_id']} (From: {issue['source_url']} To: {issue['target_url']})"
                )
            print(
                f"    Status: {'Complete' if issue.get('complete', False) else 'Incomplete' if 'complete' in issue else ''}{'Consistent' if issue.get('consistent', False) else 'Inconsistent' if 'consistent' in issue else ''}"
            )
    ...
    for res in all_db_resources:
        print(
            f"  URL: {res['url']}, Title: {res.get('metadata', {}).get('title', 'N/A')}, Content: '{res.get('content', '')[:30]}...'"
        )
